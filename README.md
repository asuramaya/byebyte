...
which only ever notifies, it never installs unattended. It does *not* touch the GNOME pill:
...
`SHA256SUMS` manifest, and its SSH signature once the trust anchor is armed — the same trust
chain a fresh install uses, written up in
