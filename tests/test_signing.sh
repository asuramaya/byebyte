...
BYEBYTE_INSTALL_SOURCED=1 source "$HERE/install.sh"
...
# principal = WHO (byebyte's identity), namespace = WHAT-FOR ("byebyte-
# release") — deliberately distinct, per RELEASE.md's identity-vs-role split
# (conflating the two was the bug alfred's review caught on siblings).
PRINCIPAL="byebyte"
NS="byebyte-release"
keyfile="$td/id_test"
...
verify_signature "$data" "$sig" "$signers" "not-byebyte" "$NS" \
